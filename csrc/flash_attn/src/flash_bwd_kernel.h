...
#include "alibi.h"
#include "flash_bwd_fp8_kernel.h"
...
__forceinline__ __device__ void compute_dq_dk_dv_1colblock(const Params &params, const int bidb, const int bidh, const int n_block) {
    ...
    Tensor sdK = make_tensor(sK.data(), typename Kernel_traits::SmemLayoutdK{});  // (SMEM_N, SMEM_K)
...
__forceinline__ __device__ void compute_dq_dk_dv(const Params &params) {
...
__forceinline__ __device__ void compute_dq_dk_dv_seqk_parallel(const Params &params) {
    ...
    for (int n_block = blockIdx.x; n_block < (params.seqlen_k + Kernel_traits::kBlockN - 1) / Kernel_traits::kBlockN; n_block += gridDim.x) {
        if constexpr (sizeof_bits_v<typename Kernel_traits::Element> == 8)
            compute_dq_dk_dv_1colblock_fp8<Kernel_traits, Is_dropout, Is_causal, Is_local, Has_alibi, Is_even_MN, Is_even_K, false, false, /*Seq_parallel=*/true>(params, bidb, bidh, n_block);
        else
