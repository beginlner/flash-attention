...
template <bool Scale_max=true, bool inplace_Scale_S=true, typename Engine0, typename Layout0, typename Engine1, typename Layout1>
__forceinline__ __device__ auto scale_apply_exp2(Tensor<Engine0, Layout0> &tensor, Tensor<Engine1, Layout1> const &max, const float scale, float Scale_S=1.0f) {
    static_assert(Layout0::rank == 2, "Only support 2D Tensor");
    static_assert(Layout1::rank == 1, "Only support 1D Tensor");
    CUTE_STATIC_ASSERT_V(size<0>(max) == size<0>(tensor));
    Tensor out = make_tensor_like(tensor);
    #pragma unroll
    for (int mi = 0; mi < size<0>(tensor); ++mi) {
        ...
        for (int ni = 0; ni < size<1>(tensor); ++ni)  {
            ...
            #endif
            if constexpr (inplace_Scale_S) tensor(mi, ni) = tensor(mi, ni) * Scale_S;
            else out(mi, ni) = tensor(mi, ni) * Scale_S;
        }
    }
    if constexpr (inplace_Scale_S) return tensor;
    else return out;
}
...
struct Softmax {
    ...
    __forceinline__ __device__ TensorT softmax_rescale_o(Tensor0 &acc_s, Tensor1 &acc_o, float softmax_scale_log2, float Scale_S=1.0f) {
        ...
        clear(scale_o);
        if (Is_first) {
            TensorT row_block_max;
            flash::template reduce_max</*zero_init=*/true>(scores, row_block_max);
            cute::copy(row_block_max, row_max);
            flash::scale_apply_exp2(scores, row_max, softmax_scale_log2, Scale_S);
            TensorT row_block_scale_P;
            for (int mi = 0; mi < size<0>(scores); ++mi) {
                row_block_scale_P(mi) = FP8_MAX;
            }
            for (int mi = 0; mi < size<0>(scores); ++mi) {
                for (int ni = 0; ni < size<1>(scores); ++ni) {
                    scores(mi, ni) = float(ElementP(scores(mi, ni) * row_block_scale_P(mi)) / row_block_scale_P(mi));
                }
            }
            ...
            TensorT row_block_max;
            flash::template reduce_max</*zero_init=*/true>(scores, row_block_max);
            for (int mi = 0; mi < size(row_max); ++mi) { row_max(mi) = max(row_max(mi), row_block_max(mi)); }
            // Reshape acc_o from (MMA=4, MMA_M, MMA_K) to (nrow=(2, MMA_M), ncol=(2, MMA_K))
            Tensor acc_o_rowcol = make_tensor(acc_o.data(), flash::convert_layout_acc_rowcol(acc_o.layout()));
    ...
    __forceinline__ __device__ TensorT normalize_softmax_lse(Tensor0 &acc_o, float softmax_scale, float rp_dropout=1.0, float Descale_S=1.0f, float Descale_V=1.0f) {
        SumOp<float> sum_op;
