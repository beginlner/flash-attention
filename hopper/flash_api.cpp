...
void run_mha_fwd_kvcache(Flash_fwd_params &params, cudaStream_t stream, bool force_split_kernel=false) { 
    int dtype = 0;
    if (params.is_bf16) { dtype = 1; }
    PREC_SWITCH(dtype, Element, [&] {
      HEADDIM_SWITCH(params.d, kHeadSize, [&] {
        run_mha_fwd_kvcache_<Element, kHeadSize>(params, stream);
      });
    });
}
