...
# for i in range(b):
#     cache_seqlens[i] = min(max(random.normalvariate(1100, 500), 1), 4096)
...
def test_flash_attention(d, v_dim):
    ...
    s_q = 1
    q = torch.randn(b, s_q, h_q, d).to(torch.float8_e4m3fn).to(dtype)
    # k = torch.randn(b, s, h_kv, d).to(torch.float8_e4m3fn).to(dtype)
    # v = k[..., :v_dim]
    ...
    blocked_k = torch.randn(block_table.numel(), block_size, h_kv, d).to(torch.float8_e4m3fn).to(dtype)
    blocked_v = blocked_k[..., :v_dim]

    q_fp8 = q.to(torch.float8_e4m3fn)
    blocked_k_fp8 = blocked_k.to(torch.float8_e4m3fn)
    blocked_v_fp8 = blocked_v.to(torch.float8_e4m3fn)
    def blocked_flash_attn():
        return flash_attn_with_blocked_kvcache(
            q_fp8, blocked_k_fp8, blocked_v_fp8, block_table, cache_seqlens, causal=True,
...
if __name__ == "__main__":
    # for d, v_dim in [(32, 32), (64, 64), (96, 96), (128, 128), (160, 160), (192, 192), (224, 224), (256, 256), (512, 512), (576, 512), (576, 576)]:
